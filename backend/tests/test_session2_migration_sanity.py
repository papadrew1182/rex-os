...
Verifies that all 14 Session 2 migration slots (008-021) are in
...
SESSION2_MIGRATIONS = [
    "008_rbac_roles_permissions.sql",
    "009_user_roles_preferences.sql",
    "010_project_assignment_bridges.sql",
    "011_connector_registry.sql",
    "012_connector_procore_stage.sql",
    "013_connector_exxir_stage.sql",
    "014_sync_runs_and_source_links.sql",
    "015_canonical_core_additions.sql",
    "016_canonical_pm_additions.sql",
    "017_canonical_financial_additions.sql",
    "018_canonical_schedule_additions.sql",
    "019_canonical_docs_quality_additions.sql",
    "020_seed_roles_and_aliases.sql",
    "021_canonical_read_views.sql",
]
...
def test_session2_migrations_in_order():
    ...
    # Verify they land in the correct order (008 before 009, ..., 020 before 021).
...
async def test_session2_rbac_tables_exist(rollback_client):
    """The four RBAC tables from 008 exist."""
...
async def test_session2_user_role_objects_exist(rollback_client):
    """009 objects: user_roles table, user_preferences table, v_users view."""
    from app.database import async_session_factory
    async with async_session_factory() as db:
...
async def test_session2_connector_schemas_exist(rollback_client):
    """012 and 013 create the two connector-specific schemas."""
    from app.database import async_session_factory
    async with async_session_factory() as db:
...
async def test_session2_connector_registry_tables_exist(rollback_client):
    """011 + 014 connector control-plane tables exist."""
    from app.database import async_session_factory
    expected = {
...
async def test_session2_source_links_view(rollback_client):
    """rex.source_links view from 014 resolves and has charter columns."""
    from app.database import async_session_factory
    async with async_session_factory() as db:
        row = (await db.execute(
...
async def test_session2_canonical_additions_exist(rollback_client):
    """016-019 net-new canonical tables exist."""
    from app.database import async_session_factory
    expected = {
